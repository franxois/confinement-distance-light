<script lang="ts">
	...
	$: precision = accuracy !== undefined ? Math.round(accuracy) + 'm' : undefined;
	...
	$: if (reference.latitude && reference.longitude && latitude && longitude) {
		distance =
			Math.round(
				calculateDistance(reference.latitude, reference.longitude, latitude, longitude) * 1000
			) + 'm';
		link2homeGmap =
...
<svelte:head>
	<title>Confinement distance light</title>
</svelte:head>
